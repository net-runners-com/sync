...
export default function SettingsPage() {
  ...
  const [xPlaywrightLoading, setXPlaywrightLoading] = useState(false);
  const [xPlaywrightResult, setXPlaywrightResult] = useState<{username?: string; error?: string} | null>(null);
  ...
  const handleConnectPlaywright = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setXPlaywrightResult({ username: data.username });
        await fetchSocialAccounts();
      } else {
        setXPlaywrightResult({ error: data.error || "不明なエラー" });
      }
    } catch (err: any) {
      setXPlaywrightResult({ error: err.message || "通信エラー" });
    } finally {
